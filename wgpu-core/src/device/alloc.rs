...
// needed because of NonNull in the allocators...
unsafe impl<B: hal::Backend> Send for MemoryAllocator<B> {}
unsafe impl<B: hal::Backend> Sync for MemoryAllocator<B> {}
...
impl<B: hal::Backend> MemoryAllocator<B> {
    pub fn new(mem_props: hal::adapter::MemoryProperties, limits: hal::Limits) -> Self {
        ...
        let properties = gpu_alloc::DeviceProperties {
            ...
            max_memory_allocation_count: limits.max_memory_allocation_count.min(!0u32 as usize)
                as u32,
    ...
    pub fn clear(&mut self, _device: &B::Device) {
        // no cleanup with gpu-alloc
    }
}

// needed because of NonNull in the allocators...
unsafe impl<B: hal::Backend> Send for MemoryBlock<B> {}
unsafe impl<B: hal::Backend> Sync for MemoryBlock<B> {}
